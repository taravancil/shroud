...
const MissingOption = zerr('MissingOption', 'opts.% is required.')
const DuplicateSecret = zerr('DuplicateSecret', 'A secret for % already exists.')
const SecretNotFound = zerr('SecretNotFound', 'No secret found for %.')
const InvalidPassword = zerr('InvalidPassword', 'Invalid master password')
...
Shroud.prototype.add = function (secretObj) {
  return new Promise((resolve, reject) => {
    ...
    this.vault.add(sealedSecretObj)
...
Shroud.prototype.reveal = function (secretName) {
  return new Promise((resolve, reject) => {
    ...
    const {sealedSecret, pubkey, salt} = secret
    try {
